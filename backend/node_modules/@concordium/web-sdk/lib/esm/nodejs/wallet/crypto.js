...
function getEncryptionMethodImport(method) {
...
function decrypt({ cipherText, metadata }, password) {
    const { keyLen, iterations, salt, initializationVector, encryptionMethod, hashAlgorithm, keyDerivationMethod } = metadata;
    ...
    const key = crypto.pbkdf2Sync(password, Buffer.from(salt, encoding), iterations, keyLen || defaultKeyLength, hashAlgorithm || hashAlgorithmInternal);
    const internalEncryptionMethod = getEncryptionMethodImport(encryptionMethod);
    const decipher = crypto.createDecipheriv(internalEncryptionMethod, key, Buffer.from(initializationVector, encoding));
 ...
 */
export function decryptMobileWalletExport(mobileWalletExport, password) {
    const decryptedMobileWalletExport = JSON.parse(decrypt(mobileWalletExport, password));
