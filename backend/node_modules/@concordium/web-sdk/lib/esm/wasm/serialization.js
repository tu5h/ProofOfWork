 ...
 */
export function getCredentialDeploymentTransactionHash(credentialDeployment, signatures) {
    const credentialDeploymentInfo = JSON.parse(wasm.getDeploymentDetails(signatures, JSONbig.stringify(credentialDeployment.unsignedCdi), credentialDeployment.expiry.expiryEpochSeconds));
 ...
 */
export function serializeCredentialDeploymentTransactionForSubmission(credentialDeployment, signatures) {
    const credentialDeploymentInfo = JSON.parse(wasm.getDeploymentDetails(signatures, JSONbig.stringify(credentialDeployment.unsignedCdi), credentialDeployment.expiry.expiryEpochSeconds));
    return Buffer.from(credentialDeploymentInfo.serializedTransaction, 'hex');
}
export function serializeCredentialDeploymentPayload(signatures, credentialDeploymentTransaction) {
    const payloadByteArray = wasm.serializeCredentialDeploymentPayload(signatures, JSONbig.stringify(credentialDeploymentTransaction.unsignedCdi));
