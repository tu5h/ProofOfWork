 ...
 */
export function getInitContractParameterSchema(moduleSchema, contractName, schemaVersion) {
    const parameterSchema = wasm.getInitContractParameterSchema(Buffer.from(moduleSchema).toString('hex'), ContractName.toString(contractName), schemaVersion);
 ...
 */
export function getUpdateContractParameterSchema(moduleSchema, contractName, receiveFunctionName, schemaVersion) {
    const parameterSchema = wasm.getReceiveContractParameterSchema(Buffer.from(moduleSchema).toString('hex'), ContractName.toString(contractName), EntrypointName.toString(receiveFunctionName), schemaVersion);
    return Buffer.from(parameterSchema, 'hex');
}
/**
 ...
 */
export function displayTypeSchemaTemplate(rawSchema) {
 ...
 */
export function serializeInitContractParameters(contractName, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/explicit-module-boundary-types
parameters, rawSchema, schemaVersion, verboseErrorMessage = false) {
    const serializedParameters = wasm.serializeInitContractParameters(JSONbig.stringify(parameters), Buffer.from(rawSchema).toString('hex'), ContractName.toString(contractName), schemaVersion, verboseErrorMessage);
 ...
 */
export function serializeUpdateContractParameters(contractName, receiveFunctionName, 
// eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/explicit-module-boundary-types
parameters, rawSchema, schemaVersion, verboseErrorMessage = false) {
    const serializedParameters = wasm.serializeReceiveContractParameters(JSONbig.stringify(parameters), Buffer.from(rawSchema).toString('hex'), ContractName.toString(contractName), EntrypointName.toString(receiveFunctionName), schemaVersion, verboseErrorMessage);
    return Parameter.fromBuffer(Buffer.from(serializedParameters, 'hex'));
}
/**
...
value, rawSchema, verboseErrorMessage = false) {
    const serializedValue = wasm.serializeTypeValue(JSONbig.stringify(value), Buffer.from(rawSchema).toString('hex'), verboseErrorMessage);
 ...
 */
export function deserializeContractState(contractName, schema, state, verboseErrorMessage = false
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
    const serializedState = wasm.deserializeState(ContractName.toString(contractName), Buffer.from(state).toString('hex'), Buffer.from(schema).toString('hex'), verboseErrorMessage);
    try {
        return JSONbig({
 ...
 * @param verboseErrorMessage Whether errors are in a verbose format or not. Defaults to `false`.
 */
export function deserializeReceiveReturnValue(returnValueBytes, moduleSchema, contractName, functionName, schemaVersion, verboseErrorMessage = false
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
    const deserializedReturnValue = wasm.deserializeReceiveReturnValue(Buffer.from(returnValueBytes).toString('hex'), Buffer.from(moduleSchema).toString('hex'), ContractName.toString(contractName), EntrypointName.toString(functionName), schemaVersion, verboseErrorMessage);
    try {
        return JSONbig({
            alwaysParseAsBig: true,
            useNativeBigInt: true,
 ...
 * @param verboseErrorMessage Whether errors are in a verbose format or not. Defaults to `false`.
 */
export function deserializeReceiveError(errorBytes, moduleSchema, contractName, functionName, verboseErrorMessage = false
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
    const deserializedError = wasm.deserializeReceiveError(Buffer.from(errorBytes).toString('hex'), Buffer.from(moduleSchema).toString('hex'), ContractName.toString(contractName), EntrypointName.toString(functionName), verboseErrorMessage);
    try {
        return JSONbig({
            alwaysParseAsBig: true,
            useNativeBigInt: true,
 ...
 * @param verboseErrorMessage Whether errors are in a verbose format or not. Defaults to `false`.
 */
export function deserializeInitError(errorBytes, moduleSchema, contractName, verboseErrorMessage = false
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
    const deserializedError = wasm.deserializeInitError(Buffer.from(errorBytes).toString('hex'), Buffer.from(moduleSchema).toString('hex'), ContractName.toString(contractName), verboseErrorMessage);
    try {
        return JSONbig({
            alwaysParseAsBig: true,
            useNativeBigInt: true,
 ...
 */
export function deserializeTypeValue(value, rawSchema, verboseErrorMessage = false) {
    const deserializedValue = wasm.deserializeTypeValue(Buffer.from(value).toString('hex'), Buffer.from(rawSchema).toString('hex'), verboseErrorMessage);
    return JSONbig({
        alwaysParseAsBig: true,
        useNativeBigInt: true,
